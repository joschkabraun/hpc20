...
int main(int argc, char *argv[]) {
  ...
  for (long k=1; k < it+1; k++) {
    ...
    // then update black points
    #pragma USE_OPENMP_FOR
    for (long i=2; i<N-1; i+=2) {
      for (long j=1; j<N-1; j+=2) {
        u_1[N*i+j] = (h*h + u_1[N*(i-1)+j] + u_1[N*i+j-1] + u_1[N*(i+1)+j] + u_1[N*i + j+1]) /4.0;
      }
    }
    for (long i=1; i<N-1; i+=2) {
      for (long j=2; j<N-1; j+=2) {
  ...
  double time = t.toc();
  printf("%f seconds per iteration", time/it);
